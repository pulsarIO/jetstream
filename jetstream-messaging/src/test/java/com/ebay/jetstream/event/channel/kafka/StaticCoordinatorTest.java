...
public class StaticCoordinatorTest {
	...
	public void testGetRebalanceCount() throws Exception {
		...
		String path0 = "/brokers/topics/" + topic0 + "/partitions";
		...
		zkConnector.create(path0 + "/3", false);
		zkConnector.create(path0 + "/4", false);

		String path1 = "/brokers/topics/" + topic1 + "/partitions";
		zkConnector.create(path1, false);
		zkConnector.create(path1 + "/0", false);
		zkConnector.create(path1 + "/1", false);
		zkConnector.create(path1 + "/2", false);
		zkConnector.create(path1 + "/3", false);
		zkConnector.create(path1 + "/4", false);

		ZkCoordinator zkCoordinator0 = new StaticCoordinator("a,-1,1,2,3",
				topics1, groupId1, consumerId0, zkConnector);
		ZkCoordinator zkCoordinator1 = new StaticCoordinator("0,4,5", topics1,
		...
		assertTrue(topicCount0.get(topic1) == 3);
		assertTrue(topicCount1.get(topic0) == 2);
		assertTrue(topicCount1.get(topic1) == 2);

		zkCoordinator0.takePartition(topic0, 1);
		zkCoordinator1.takePartition(topic1, 0);
		zkCoordinator1.takePartition(topic1, 2);
		zkCoordinator1.takePartition(topic1, 4);

		topicCount0 = zkCoordinator0.getRebalanceCount();
		topicCount1 = zkCoordinator1.getRebalanceCount();
		assertTrue(topicCount0.get(topic0) == 2);
		assertTrue(topicCount0.get(topic1) == 3);
		assertTrue(topicCount1.get(topic0) == 2);
		assertTrue(topicCount1.get(topic1) == -1);
	...
	public void testGetToTakePartitions() throws Exception {
		...
		String path2 = "/brokers/topics/" + topic2 + "/partitions";
		zkConnector.create(path2, false);
		zkConnector.create(path2 + "/0", false);
		zkConnector.create(path2 + "/1", false);
		zkConnector.create(path2 + "/2", false);

		ZkCoordinator zkCoordinator2 = new StaticCoordinator("a,-1,1,2,5",
				topics2, groupId2, consumerId2, zkConnector);

		Set<Integer> toTake = zkCoordinator2.getToTakePartitions(topic2);
		assertEquals(2, toTake.size());
		assertTrue(toTake.contains(1));
		assertTrue(toTake.contains(2));

		zkCoordinator2.takePartition(topic2, 1);
	...
	public void testGetToReleasePartitions() throws Exception {
		...
		String path3 = "/brokers/topics/" + topic3 + "/partitions";
		zkConnector.create(path3, false);
		zkConnector.create(path3 + "/0", false);
		zkConnector.create(path3 + "/1", false);
		zkConnector.create(path3 + "/2", false);
		zkConnector.create(path3 + "/3", false);

		ZkCoordinator zkCoordinator3 = new StaticCoordinator("a,-1,1,2,5",
				topics3, groupId3, consumerId3, zkConnector);

		zkCoordinator3.takePartition(topic3, 0);
		zkCoordinator3.takePartition(topic3, 1);
		zkCoordinator3.takePartition(topic3, 2);
		zkCoordinator3.takePartition(topic3, 3);

		Set<Integer> toRelease = zkCoordinator3.getToReleasePartitions(topic3,
				2);
